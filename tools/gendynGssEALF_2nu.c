...
#define ntry_th 31250  // ntry per thread; ntry = N_TH*ntry_th = 1E6
...
void *calc_pol(void *arg)
{
  ...
  for(k=0;k<ntry_th;k++){
    ...
    for(i=0;i<ndiv;i++){
      if( gsl_rng_uniform(r_global2[id]) < (1.0-exp(-nu2*dt)) ){ //Poisson's distribution (n!=0)
	multivariate_gaussian(sigma_sta,Bint_sta,id);
	multivariate_gaussian(sigma,Bint,id);
	gsl_vector_memcpy(Btot,Bint);
	gsl_vector_add(Btot,Bint_sta);
	gsl_vector_add(Btot,Bext);
      ...
      else if( gsl_rng_uniform(r_global2[id]) < (1.0-exp(-nu1*dt)) ){ //Poisson's distribution (n!=0)
	multivariate_gaussian(sigma,Bint,id);
	gsl_vector_memcpy(Btot,Bint);
	gsl_vector_add(Btot,Bint_sta);
	gsl_vector_add(Btot,Bext);
...
int main(int argc, char *argv[]){
  ...
  if(argc !=6){
    printf("usage: $this Delta(us-1) Q nu_{ion}/Delta LF/(Delta/gamma) nu_{mu}/Delta\n");
    exit(1);
  }
  ...
  //
  char fname[128];
  sprintf(fname, "%s%s%s%s%s%s%s%s%s%s", argv[1], "-" ,argv[2], "-", argv[3], "-", argv[4], "-", argv[5], ".dat");
  FILE *fp;
  fp = fopen(fname,"w");
